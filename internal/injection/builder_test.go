...
func TestBuildPrompt_TeamLeadWithMembers(t *testing.T) {
	...
	// v3 phase A: file message bus removed; SendMessage no longer
	// appears in the per-team prompt.
	if strings.Contains(prompt, "SendMessage") {
		t.Errorf("prompt should not mention SendMessage after bus removal:\n%s", prompt)
	}
	if strings.Contains(prompt, "Message Bus") {
		t.Errorf("prompt should not mention Message Bus after bus removal:\n%s", prompt)
	}
}
